...
sed -i -- 's+Exec=/usr/bin/google-chrome-stable %U+Exec=/usr/bin/google-chrome-stable %U --make-default-browser --disable-metrics+g'\Exec=/usr/bin/google-chrome-stable %U https://'$DOMAIN'\Exec=/usr/bin/google-chrome-stable %U --make-default-browser --disable-metrics+g' /etc/xdg/autostart/google-chrome.desktop
#Reboots system
reboot
